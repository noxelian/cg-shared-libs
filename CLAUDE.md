...
| `kafka` | Kafka producer + consumer wrappers (segmentio/kafka-go under the hood). Producers are synchronous and require acknowledgements from all in-sync replicas before `Publish` succeeds. With DLQ enabled, the source offset is committed only after an acknowledged DLQ write; DLQ outages retain the offset until recovery or shutdown. Consumers handling sensitive schemas can use `NewConsumerWithOptions(..., WithDLQValueRedactor(...))`; redaction errors retain the source offset and never fall back to raw bytes. |
